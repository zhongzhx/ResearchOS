...
let activeTab = "Tasks";
const tabs = ["Tasks", "SkillRuns", "Execution Memory", "Agent Feed", "Workflow Board"];
...
export async function renderRunsView({ root }) {
  ...
  root.querySelector("#runsContent").innerHTML = `
    ${tabButtons()}
    ${result.ok ? renderRows(rows[activeTab] || [], `No ${activeTab.toLowerCase()} yet`) : apiErrorCard(result, `${activeTab} endpoint unavailable`)}
