...
def _download_priority(record: dict[str, Any]) -> int:
    """Rank records so basic download runs try likely full-text candidates first."""
    if _publisher_pdf_candidates(str(record.get("doi") or "")):
        return 0
    pdf_url = str(record.get("pdf_url") or "").lower()
    fulltext_url = str(record.get("fulltext_url") or "").lower()
    landing_page_url = str(record.get("landing_page_url") or "").lower()
    if pdf_url.startswith("http") and ("pmc.ncbi.nlm.nih.gov" in pdf_url or "europepmc.org" in pdf_url):
        return 1
    if pdf_url.startswith("http"):
        return 2
    if fulltext_url.startswith("http") and ("pmc.ncbi.nlm.nih.gov" in fulltext_url or "europepmc.org" in fulltext_url):
        return 3
    if fulltext_url.startswith("http"):
        return 4
    if _is_truthy(record.get("open_access_flag")):
        return 5
...
def _sort_records_for_download(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    indexed = list(enumerate(records))
    indexed.sort(key=lambda item: (_download_priority(item[1]), item[0]))
    return [record for _, record in indexed]


def _publisher_pdf_candidates(doi: str) -> list[str]:
    """Return official publisher PDF URLs that can be derived safely from DOI."""
    doi = doi.strip().lower()
    candidates: list[str] = []
...
def _candidate_download_urls(row: dict[str, Any]) -> list[str]:
    urls: list[str] = []
    urls.extend(_publisher_pdf_candidates(str(row.get("doi") or "")))
...
def cmd_harvest(args: argparse.Namespace) -> None:
    ...
    if not scored_records:
        scored_records = _sort_records_for_download(df.to_dict(orient="records"))
    ...
    if args.score_only or not args.download:
    ...
    for idx, row in enumerate(scored_records[:limit], start=1):
        ...
        oa_urls = _candidate_download_urls(row)
        downloaded = False
        oa_html_status = ""
        oa_html_path = ""
        oa_html_url = ""
        ...
        if not downloaded:
            ...
            if oa_html_status:
                ...
                dr.download_status = DownloadStatus.MANUAL_DOWNLOAD_REQUIRED.value
                dr.failure_reason = dr.failure_reason or "no_auto_download_route"
...
def cmd_resume(args: argparse.Namespace) -> None:
    ...
    for idx, candidate in enumerate(to_process, start=1):
        ...
        # 1: OA download
        oa_urls = _candidate_download_urls(candidate)
        for oa_url in oa_urls:
