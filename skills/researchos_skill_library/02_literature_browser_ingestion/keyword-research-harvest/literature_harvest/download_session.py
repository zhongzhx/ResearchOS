...
class DownloadSession:
    ...
    def extract_pdf_url_from_html(self, html_text: str, base_url: str) -> str:
        """Extract a likely article PDF URL from an HTML landing page."""
        soup = BeautifulSoup(html_text, "html.parser")

        citation_pdf = soup.find("meta", attrs={"name": "citation_pdf_url"})
        if citation_pdf:
            content = citation_pdf.get("content") or ""
            if content:
    ...
    def download_to_file(
        ...
        if kind == "html":
            ...
            pdf_url = self.extract_pdf_url_from_html(html_text, final_url)
            if pdf_url and pdf_url != final_url:
                pdf_response = self.attempt_download(pdf_url)
                pdf_payload = pdf_response["payload"]
                pdf_error = pdf_response["error"]
                pdf_final_url = pdf_response["final_url"]
                pdf_kind = self.classify_payload(pdf_payload, pdf_response["content_type"])
                if not pdf_error and pdf_kind == "pdf":
                    output_path = Path(output_path)
                    save_path = output_path.with_suffix(".pdf")
                    saved = self.save_payload(pdf_payload, save_path)
                    if self.check_pdf_magic(save_path):
                        return {"status": OA_PDF_DOWNLOADED.value, "reason": "",
                                "content_format": "pdf", "path": saved, "url": pdf_final_url}
